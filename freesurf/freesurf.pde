...
void draw() {
  ...
  case 0: //Waiting for player to stand over board
    ...
    drawboardaux(vector_player.x,vector_player.y,255,255,255,255);
    break;
  case 1: //player playing
    ...
    if (time_game<=0){
      state=3;
      println("lack of time, loose");
    }

    if (score==10){ //goes to winning score
      state=5;
      println("win");
    }
    break;
  case 3: //wipeout
    ...
    if ((vector_player.y)>20) {//if (feetsensor==1) {
      play();
      state=4;
      println("from wipe out to transition");
    }     
    break;
  case 4: //transition state before game goes back to initial page, player must step up and down once, (this can be implemented with a timer)
    ...
    textFont(font_game, 50);
    text("Wipe-out!!", width*5/10, height*8/10); 
    if ((vector_player.y)<20) {
      state=0;
      println("from transition to start");
    }     
    break;
  case 5: 
